...
class EliTwonky(Player):
    def __init__(self, deck, game, turnOrder, name='EliTwonky', character='Marsh'):
        super().__init__(deck, game, turnOrder, name, character)
        self.job = ''
        self.seekCount = 0
        self.unlearnt = False
        charData = {"Kelsier": 0.04, "Marsh": 0.25, "Shan": -0.16, "Vin": 0.09, "Prodigy": -0.16}
    ...
    def selectAction(self, actions, game):
        ...
        handScores = sorted([(card, self.sorting_algo(card)) for card in self.deck.hand], key=lambda x:x[1], reverse=True)
        for card, score in handScores:
            if isinstance(card, Action) and (not card.burned) and card.metalUsed < card.capacity:
                if (4, card) in actions:
                    return (4, card)
