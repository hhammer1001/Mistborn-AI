...
export class Market extends Deck {
  ...
  private _buildTestDeck() {
    ...
    const pickpocketDef: CardDef = {
      cardType: 3, name: "Pickpocket", cost: 3, metal: 6,
      ability1Effect: "M", ability1Amount: "1",
      ability2Effect: "pull", ability2Amount: "1",
      health: 2,
    };
    const preserveDef: CardDef = {
      cardType: 2, name: "Preserve", cost: 4, metal: 8,
      ability1Effect: "Mi.D", ability1Amount: "5.2",
    };
    for (let i = 0; i < 10; i++) {
      this.cards.push(createCard(houselordDef));
      this.cards.push(createCard(pickpocketDef));
      this.cards.push(createCard(preserveDef));
    }
  }
