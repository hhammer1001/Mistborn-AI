...
export class Player {
  ...
  special11() { // Maelstrom: kill all opponent allies + clear market
    ...
    // Trash the cards currently in market.hand (snapshot first — market.buy
    // refills hand from cards/discard, so iterating market.hand directly is
    // an infinite loop). Move each to market.discard, then refill hand once
    // by drawing the original count.
    const cleared = [...this.game.market.hand];
    this.game.market.hand = [];
    for (const card of cleared) {
      this.game.market.discard.push(card);
    }
    this.game.market.draw(cleared.length);
  }
